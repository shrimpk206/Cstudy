...
각 레슨에는 5문제짜리 인터랙티브 연습이 붙어 있습니다 (자료 추가 진행 중).
- 5가지 문제 유형: 객관식, 단어 골라 채우기, 줄 순서, 버그 찾기, 다중 선택 체크리스트
- 정답마다 +10 XP, 5/5 첫 시도 정답이면 ★ 완벽 + 보너스 +20
- 일일 스트릭 (주 1회 동결권 자동)
- 하트/생명 없음 — 오답도 진행, 부드럽게
- 레슨 상태: ○ 읽음 / ✓ 통과(4+/5) / ★ 완벽(5/5 첫 시도)

문제 파일은 `questions/phase{2,3,4}/{XX}.json`에 있고, 직접 수정 가능합니다.
